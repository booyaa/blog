...
So nom ended up as a macros based DSL, that would generate code depending on
...
a parser's result in an `Option` (using `None` if there's an error), is
defined:
...
At the same time, macros had issues that followed nom for a long time.
...
harder to catch mistakes). Argument parsing was limited in the way some
...
the infromation you need. You can learn more about it in the
...
of parser results, that indicates the parser cannot decides and needs more data.
...
they inexplicably made parsers slower.
...
This way, you can choose explicitely hich version you use, and can even use
...
We now use the [lexical crate](https://crates.io/crates/lexical-core) for
...
shows new pattern that are available in nom 5. It is now easy to make an iterator
...
To get proper results, I'd advise two tips:
- using [jemallocator](https://crates.io/crates/jemallocator) instead of the
system allocator. Test with and without it and see how it affects the results
- use link time optimization (LTO) when compiling, which will reduce code size
